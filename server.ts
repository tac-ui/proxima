...
async function main() {
  ...
  const httpServer = createServer((req, res) => {
    // Record the real TCP peer address for local-only auth gating. Strip any
    // client-supplied value first so the header can't be spoofed from outside.
    delete req.headers["x-proxima-remote-addr"];
    const remoteAddr = req.socket.remoteAddress;
    if (remoteAddr) req.headers["x-proxima-remote-addr"] = remoteAddr;
